...
class LabelEmbed:
    def __init__(self, dataset, path_y2h, path_y2cov, y2h_type="resnet", y2cov_type="sinusoidal", h_dim = 128, cov_dim = 64**2*3, batch_size=128, nc=3):
        ...
        if y2h_type == "resnet":
            ...
            state_dict = checkpoint["net_state_dict"]
            # 兼容DDP,如果 key 里带有 "module." 前缀，就全部去掉
        ...
        if y2cov_type == "resnet":
            ...
            if accelerator.is_main_process and not os.path.isfile(mlp_y2cov_filename_ckpt):
                model_mlp_y2cov = train_mlp(unique_labels_norm = unique_labels_norm, model_mlp=model_mlp_y2cov, model_name="mlp_y2cov", model_h2y=model_h2y, epochs=500, lr_base=base_lr_mlp, lr_decay_factor=0.1, lr_decay_epochs=[150, 250, 350], weight_decay=1e-4, batch_size=128, device=accelerator.device)
                # save model
                torch.save({
...
def train_resnet(net, net_name, trainloader, epochs=200, resume_epoch = 0, lr_base=0.01, lr_decay_factor=0.1, lr_decay_epochs=[80, 140], weight_decay=1e-4, path_to_ckpt = None):
    ...
    for epoch in range(resume_epoch, epochs):
        ...
        accelerator.print('Train {} for embedding: [epoch {}/{}] train_loss:{:.4f} Time:{:.4f}'.format(net_name, epoch+1, epochs, train_loss, timeit.default_timer()-start_tmp))

        #save checkpoint
        if accelerator.is_main_process and (path_to_ckpt is not None and (((epoch+1) % 50 == 0) or (epoch+1==epochs))):
...
def train_mlp(unique_labels_norm, model_mlp, model_name, model_h2y, epochs=500, lr_base=0.01, lr_decay_factor=0.1, lr_decay_epochs=[150, 250, 350], weight_decay=1e-4, batch_size=128):
    '''
