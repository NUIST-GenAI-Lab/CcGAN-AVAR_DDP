...
class Evaluator:
    def __init__(self, dataset, trainer, args, device):
        ...
        if self.data_name == "UTKFace":
            ...
            if args.img_size == 64:
                ...
                self.fid_net_path = "./evaluation/eval_ckpts/UTKFace/metrics_{}x{}/ckpt_AE_epoch_200_seed_2021_CVMode_False.pth".format(
                    args.img_size, args.img_size)
                self.class_net_path = "./evaluation/eval_ckpts/UTKFace/metrics_{}x{}/ckpt_PreCNNForEvalGANs_ResNet34_class_epoch_200_seed_2021_classify_5_races_CVMode_False.pth".format(
                    args.img_size, args.img_size)
                self.regre_net_path = "./evaluation/eval_ckpts/UTKFace/metrics_{}x{}/ckpt_PreCNNForEvalGANs_ResNet34_regre_epoch_200_seed_2021_CVMode_False.pth".format(
                    args.img_size, args.img_size)
            ...
            self.fid_net_path = "./evaluation/eval_ckpts/SteeringAngle/metrics_{}x{}/ckpt_AE_epoch_200_seed_2020_CVMode_False.pth".format(
                args.img_size, args.img_size)
            self.class_net_path = "./evaluation/eval_ckpts/SteeringAngle/metrics_{}x{}/ckpt_PreCNNForEvalGANs_ResNet34_class_epoch_20_seed_2020_classify_5_scenes_CVMode_False.pth".format(
                args.img_size, args.img_size)
            self.regre_net_path = "./evaluation/eval_ckpts/SteeringAngle/metrics_{}x{}/ckpt_PreCNNForEvalGANs_ResNet34_regre_epoch_200_seed_2020_CVMode_False.pth".format(
                args.img_size, args.img_size)
        ...
        print("Dataset: {}; Resolution: {}x{}; num_classes: {}".format(self.data_name, self.img_size, self.img_size,
                                                                       self.num_classes))
        ...
        if args.do_subsampling and args.use_dre_reg:
            self.fake_images, _ = self.trainer.rejection_sampling_given_labels(
                dataset.fn_normalize_labels(self.fake_labels), self.nburnin_per_label, batch_size=args.samp_batch_size,
                verbose=True)
        else:
            self.fake_images, _ = self.trainer.sample_given_labels(dataset.fn_normalize_labels(self.fake_labels),
                                                                   batch_size=args.samp_batch_size, denorm=True,
                                                                   to_numpy=True, verbose=True)
        stop = timeit.default_timer()
        assert self.fake_images.max() > 1.0
        print("\r {}/{}: Got {} fake images. Time spent {:.2f} sec.".format(i + 1, self.num_eval_labels,
                                                                            len(self.fake_images), stop - start))
    ...
    def compute_metrics(self, output_path, PreNetFID, PreNetDiversity, PreNetLS):
        ...
        # be compatible with DP or not
        state = checkpoint_PreNet['net_encoder_state_dict']
        new_state = {}
        for k, v in state.items():
            if k.startswith("module."):
                ...
                new_k = k
            new_state[new_k] = v
        PreNetFID.load_state_dict(new_state)
        # for Diversity
        ...
        for i in range(len(centers_loc)):
            ...
            # FID
            FID_over_centers[i] = cal_FID(PreNetFID, real_images_i, fake_images_i, batch_size=self.args.eval_batch_size,
                                          resize=None, device=self.device)
            # Entropy of predicted class labels
            if self.data_name in ["UTKFace", "RC-49", "RC-49_imb", "SteeringAngle"]:
                predicted_class_labels = predict_class_labels(PreNetDiversity, fake_images_i,
                                                              batch_size=self.args.eval_batch_size,
                                                              num_workers=self.args.num_workers, device=self.device)
                entropies_over_centers[i] = compute_entropy(predicted_class_labels)
            # Label score
            labelscores_over_centers[i], _ = cal_labelscore(PreNetLS, fake_images_i,
                                                            self.dataset.fn_normalize_labels(fake_labels_assigned_i),
                                                            min_label_before_shift=self.min_label_before_shift,
                                                            max_label_after_shift=self.max_label_after_shift,
                                                            batch_size=self.args.eval_batch_size, resize=None,
                                                            num_workers=self.args.num_workers, device=self.device)

            print("\n Center:{}; Real:{}; Fake:{}; FID:{:.3f}; LS:{:.3f}; ET:{:.3f}; Time:{:.3f}.".format(center,
                                                                                                          len(real_images_i),
                                                                                                          len(fake_images_i),
                                                                                                          FID_over_centers[
                                                                                                              i],
                                                                                                          labelscores_over_centers[
                                                                                                              i],
                                                                                                          entropies_over_centers[
                                                                                                              i],
                                                                                                          timeit.default_timer() - start_time))
        ##end for i

        # average over all centers
        print("\n SFID: {:.3f} ({:.3f}); min/max: {:.3f}/{:.3f}. \n".format(np.mean(FID_over_centers),
                                                                            np.std(FID_over_centers),
                                                                            np.min(FID_over_centers),
                                                                            np.max(FID_over_centers)))
        print(
            "\n LS over centers: {:.3f} ({:.3f}); min/max: {:.3f}/{:.3f}. \n".format(np.mean(labelscores_over_centers),
        ...
        print("\n Entropy over centers: {:.3f} ({:.3f}); min/max: {:.3f}/{:.3f}. \n".format(
            np.mean(entropies_over_centers), np.std(entropies_over_centers), np.min(entropies_over_centers),
        ...
        ls_mean_overall, ls_std_overall = cal_labelscore(PreNetLS, self.fake_images,
                                                         self.dataset.fn_normalize_labels(self.fake_labels),
                                                         min_label_before_shift=self.min_label_before_shift,
                                                         max_label_after_shift=self.max_label_after_shift,
                                                         batch_size=self.args.eval_batch_size, resize=None,
                                                         norm_img=True, num_workers=self.args.num_workers,
                                                         device=self.device)
        ...
        FID = cal_FID(PreNetFID, self.real_images[indx_shuffle_real], self.fake_images[indx_shuffle_fake],
                      batch_size=self.args.eval_batch_size, resize=None, norm_img=True, device=self.device)
        print("FID of {} fake images: {:.3f}.\n".format(nfake_all, FID))

        #####################
        # Compute IS
        if self.data_name != "Cell200":
            IS, IS_std = inception_score(imgs=self.fake_images[indx_shuffle_fake], num_classes=self.num_classes,
                                         net=PreNetDiversity, batch_size=self.args.eval_batch_size, splits=10,
                                         normalize_img=True, device=self.device)
    ...
    def dump_h5_files(self, output_path):
        ...
        for i in range(self.num_eval_labels):
            ...
            print('\r [{}/{}]: Got {} fake images for label {} >>>'.format(i + 1, self.num_eval_labels, len(indx_i),
                                                                           label_i))
            dump_fake_images_filename = os.path.join(output_path, '{}.h5'.format(label_i))
            with h5py.File(dump_fake_images_filename, "w") as f:
                f.create_dataset('fake_images_i', data=fake_images_i, dtype='uint8', compression="gzip",
                                 compression_opts=6)
