...
class Trainer(object):
    def __init__(
        ...
        self.min_abs_diff = np.min(np.abs(np.diff(
            np.sort(self.unique_train_labels))))  # Compute the minimum absolute difference between adjacent elements.
        ...
        self.netG, self.netD, self.optG, self.optD = self.accelerator.prepare(self.netG, self.netD, self.optG,
                                                                              self.optD)
        ...
        if self.accelerator.is_main_process:
            if self.use_ema:
                self.ema_g = EMA(netG, update_after_step=ema_update_after_step, beta=ema_decay,
    ...
    def make_vicinity(self, batch_target_labels, batch_target_labels_in_dataset):
        ...
        if not self.vicinal_params["use_ada_vic"]:
            ...
            batch_real_indx = np.zeros(self.batch_size_disc,
                                       dtype=int)  # the indices of selected real images in the training dataset; the labels of these images are in the vicinity
            ...
            for j in range(self.batch_size_disc):
                ...
                if self.vicinal_params["threshold_type"] == "hard":
                    ...
                    indx_real_in_vicinity = np.where((self.train_labels - batch_target_labels[j]) ** 2 <= -np.log(
                        self.vicinal_params["nonzero_soft_weight_threshold"]) / self.vicinal_params["kappa"])[0]
                ...
                if self.vicinal_params["threshold_type"] == "hard":
                    ...
                    lb = batch_target_labels[j] - np.sqrt(
                        -np.log(self.vicinal_params["nonzero_soft_weight_threshold"]) / self.vicinal_params["kappa"])
            ...
            if self.vicinal_params["threshold_type"] == "hard":
                ...
                real_weights = torch.exp(
                    -self.vicinal_params["kappa"] * (batch_real_labels - batch_target_labels) ** 2).to(self.device)
                fake_weights = torch.exp(
                    -self.vicinal_params["kappa"] * (batch_fake_labels - batch_target_labels) ** 2).to(self.device)

            kappa_l_all = np.ones(self.batch_size_disc) * self.vicinal_params[
                "kappa"]  # the left radii of the vicinity for the target labels
            kappa_r_all = np.ones(self.batch_size_disc) * self.vicinal_params[
                "kappa"]  # the right radii of the vicinity for the target labels
        ...
        ## adaptive vicinity
            ...
            kappa_r_all = np.zeros(
                self.batch_size_disc)  # the right radii of the vicinity for the target labels
            for j in range(self.batch_size_disc):
                ...
                ## case 3: other cases
                    ...
                    dist2left = np.abs(target_y - self.unique_train_labels[
                ...
                indx_real_in_soft_vicinity_left = np.where((self.train_labels - target_y) ** 2 <= -np.log(
                    self.vicinal_params["nonzero_soft_weight_threshold"]) / nu_l)[0]
                indx_real_in_soft_vicinity_left = np.intersect1d(indx_real_in_soft_vicinity_left, indx_left)
                indx_real_in_soft_vicinity_right = np.where((self.train_labels - target_y) ** 2 <= -np.log(
                    self.vicinal_params["nonzero_soft_weight_threshold"]) / nu_r)[0]
                indx_real_in_soft_vicinity_right = np.intersect1d(indx_real_in_soft_vicinity_right, indx_right)
                indx_real_in_soft_vicinity = np.concatenate(
                    [indx_real_in_soft_vicinity_left, indx_real_in_soft_vicinity_right])
                if self.vicinal_params["ada_vic_type"].lower() == "vanilla":
                    ...
                    indx_real_in_vicinity = np.intersect1d(indx_real_in_hard_vicinity,
                                                           indx_real_in_soft_vicinity)  # soft in hard, smaller vicinity
                else:
                ...
                ub_hard = min(ub_hard, 1.0)
                assert lb_hard <= ub_hard and lb_hard >= 0 and lb_hard <= 1 and ub_hard >= 0 and ub_hard <= 1
                lb_soft = batch_target_labels[j] - np.sqrt(
                    -np.log(self.vicinal_params["nonzero_soft_weight_threshold"]) / nu_l)
                ub_soft = batch_target_labels[j] + np.sqrt(
                    -np.log(self.vicinal_params["nonzero_soft_weight_threshold"]) / nu_r)
                lb_soft = max(0.0, lb_soft);
                ub_soft = min(ub_soft, 1.0)
    ...
    def fn_disc_adv_loss(self, real_adv_out, fake_adv_out, real_weights=None, fake_weights=None, eps=1e-20):
        ...
        loss = torch.mean(real_weights.view(-1) * d_loss_real.view(-1)) + torch.mean(
    ...
    def fn_disc_aux_reg_loss(self, real_gt_labels, real_pred_labels, fake_gt_labels, fake_pred_labels, epsilon=0):
        if self.aux_loss_params['aux_reg_loss_type'].lower() in ['mse']:
            reg_loss = torch.mean((real_pred_labels.view(-1) - real_gt_labels.view(-1)) ** 2) + torch.mean(
                (fake_pred_labels.view(-1) - fake_gt_labels.view(-1)) ** 2)
            ...
            reg_loss = torch.mean(
                torch.clamp(real_abs_diff - epsilon * self.aux_loss_params['aux_reg_loss_ei_hinge_factor'],
                            min=0)) + torch.mean(
                torch.clamp(fake_abs_diff - epsilon * self.aux_loss_params['aux_reg_loss_ei_hinge_factor'], min=0))
        elif self.aux_loss_params['aux_reg_loss_type'].lower() in ['huber']:
            huber_loss_real = self.adaptive_huber_loss(real_pred_labels.view(-1), real_gt_labels.view(-1),
                                                       quantile=self.aux_loss_params["aux_reg_loss_huber_quantile"],
                                                       delta=self.aux_loss_params["aux_reg_loss_huber_delta"],
                                                       reduction="mean")
            huber_loss_fake = self.adaptive_huber_loss(fake_pred_labels.view(-1), fake_gt_labels.view(-1),
                                                       quantile=self.aux_loss_params["aux_reg_loss_huber_quantile"],
                                                       delta=self.aux_loss_params["aux_reg_loss_huber_delta"],
                                                       reduction="mean")
    ...
    def penalized_softplus_loss(self, dr_real, dr_fake):
        ...
        SP_div = torch.mean(sigmoid_fn(dr_fake) * dr_fake) - torch.mean(softplus_fn(dr_fake)) - torch.mean(
            sigmoid_fn(dr_real))
        ...
        while self.step < self.niters:
            ...
            for _ in range(self.num_D_steps):
                ...
                for accumulation_index in range(self.num_grad_acc_d):
                    ...
                    batch_target_labels_in_dataset = np.random.choice(self.unique_train_labels,
                                                                      size=self.batch_size_disc, replace=True)
                    ...
                    ## make vicinity
                    batch_real_indx, batch_fake_labels, batch_real_labels, real_weights, fake_weights, kappa_l_all, kappa_r_all = self.make_vicinity(
                        batch_target_labels, batch_target_labels_in_dataset)
                    ...
                    batch_real_images = torch.from_numpy(
                        normalize_images(batch_real_images, to_neg_one_to_one=True)).type(torch.float).to(device)
                    ...
                    with self.accelerator.autocast():
                        # forward pass
                        if self.use_diffaug:
                            real_disc_out_dict = self.netD(DiffAugment(batch_real_images, policy=self.diffaug_policy),
                                                           self.fn_y2h(batch_target_labels))
                            fake_disc_out_dict = self.netD(
                                DiffAugment(batch_fake_images.detach(), policy=self.diffaug_policy),
                                self.fn_y2h(batch_target_labels))
                        else:
                        ...
                        ## compute loss
                        ### adversarial loss
                        d_adv_loss = self.fn_disc_adv_loss(real_adv_out=real_disc_out_dict['adv_output'],
                                                           fake_adv_out=fake_disc_out_dict['adv_output'],
                                                           real_weights=real_weights, fake_weights=fake_weights)
                        d_adv_loss_val = d_adv_loss.item()
                        d_loss = d_adv_loss

                        ### auxiliary regression loss
                        if self.aux_loss_params["weight_d_aux_reg_loss"] > 0 and self.aux_loss_params[
                            "use_aux_reg_branch"]:
                            if self.aux_loss_params["use_aux_reg_model"]:
                            ...
                            d_reg_loss = self.fn_disc_aux_reg_loss(real_gt_labels=batch_target_labels,
            ...
            for _ in range(self.num_grad_acc_g):
                ...
                batch_target_labels_in_dataset = np.random.choice(self.unique_train_labels, size=self.batch_size_gene,
                                                                  replace=True)
                ## add Gaussian noise; we estimate image distribution conditional on these labels
                ...
                with self.accelerator.autocast():
                    ...
                    # g loss
                    if self.use_diffaug:
                        disc_out_dict = self.netD(DiffAugment(batch_fake_images, policy=self.diffaug_policy),
                                                  self.fn_y2h(batch_target_labels))
                    else:
                    ...
                    if self.aux_loss_params["weight_g_aux_reg_loss"] > 0 and (
                        ...
                        elif self.aux_loss_params[
                            "use_aux_reg_model"]:  # if regression branch is disabled, then try to use aux regression model to predict labels
                            fake_pred_labels = self.aux_reg_net(batch_fake_images)
                        g_reg_loss = self.fn_gene_aux_reg_loss(fake_gt_labels=batch_target_labels,
    ...
    def sample_given_labels(self, given_labels, batch_size, denorm=True, to_numpy=False, verbose=False):
        ...
        with torch.inference_mode():
            ...
            while n_img_got < nfake:
                ...
                y = torch.from_numpy(fake_labels[n_img_got:(n_img_got + batch_size)]).type(torch.float).view(-1, 1).to(
                    device)
                if self.use_ema:
    ...
    def finetune_cdre(self):
        ...
        for step in range(dre_ft_niters):
            ...
            ## draw real image/label batch from the training set
            batch_real_images = self.fn_transform(self.train_images[batch_real_indx])
            batch_real_images = torch.from_numpy(normalize_images(batch_real_images, to_neg_one_to_one=True)).type(
    ...
    def rejection_sampling_given_label(self, given_label, nfake, nburnin, batch_size=100, verbose=False):
        ...
        burnin_images, _ = self.sample_given_labels(given_labels=burnin_labels,
                                                    batch_size=np.min([batch_size, nburnin]), denorm=True,
        ...
        while num_imgs < nfake:
            batch_images, batch_labels = self.sample_given_labels(given_labels=np.ones(batch_size) * given_label,
                                                                  batch_size=batch_size, denorm=True, to_numpy=True,
                                                                  verbose=False)
            batch_ratios = self.compute_density_ratio(images=batch_images, labels=batch_labels, batch_size=batch_size,
                                                      to_numpy=True)
