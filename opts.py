...
def parse_opts():
    ...
    ''' Dataset '''
    parser.add_argument('--data_name', type=str, default='RC-49_imb',
                        choices=["RC-49", "UTKFace", "Cell200", "SteeringAngle", "RC-49_imb", "Cell200_imb"])
    parser.add_argument('--imb_type', type=str, default='unimodal',
                        choices=['unimodal', 'dualmodal', 'trimodal', 'standard', 'none'])  # none means using all data
    ...
    parser.add_argument('--use_sn', action='store_true',
                        default=False)  # use spectral normalization, effective for biggan
    ...
    parser.add_argument('--ada_vic_type', type=str, default='vanilla',
                        choices=["vanilla", "hybrid"])  # adaptive vicinity type
    ...
    parser.add_argument('--use_aux_reg_branch', action='store_true',
                        default=False)  # whether discriminator has an extra branch for regression
    parser.add_argument('--use_aux_reg_model', action='store_true',
                        default=False)  # whether we need a pre-trained independent model for regression
    parser.add_argument('--aux_reg_loss_type', type=str, default='huber',
                        choices=['mse', 'ei_hinge', 'huber'])  # mse, vicinal mse, epsilon-insensitive hinge loss
    parser.add_argument('--aux_reg_loss_ei_hinge_factor', type=float, default=1.0)
    parser.add_argument('--aux_reg_loss_huber_delta', type=float,
                        default=-1)  # if negative, then use adaptive huber loss
    ...
    parser.add_argument('--do_dre_ft', action='store_true',
                        default=False)  # finetune the DRE branch of the discriminator
    parser.add_argument('--dre_ft_niters', type=int, default=1000,
                        help='number of iterations for finetuning DRE branch')
